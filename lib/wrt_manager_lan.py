...
class WrtLanManager:
    ...
    def __init__(self, param):
        ...
        if self.param.config.wanConnection is not None:
            self.intfList.remove(self.param.config.wanConnection.interface)
        ...
        with open("/etc/resolv.conf", "w") as f:
            f.write("# Generated by fpemud-wrt\n")
    ...
    def _runHostapd(self):
        if len(self.param.config.wifiNetworks) == 0:
            return
        ...
        for i in range(0, len(wlanIntfList)):
            ...
            buf += "\n"
            for j in range(0, len(self.param.config.wifiNetworks)):
                wifiNet = self.param.config.wifiNetworks[j]
                buf += "# AP %d\n" % (j + 1)
                if j > 0:
    ...
    def _addCgfwNatRules(self):
        ...
        try:
            ...
            self.cgfwNatRuleHandle = WrtUtil.nftAddRule("fpemud-wrt", "natpost", "oif \"%s\" masquerade" % (cgfwInterface))
            return True
        except:
            return False
    ...
    def _removeCgfwNatRules(self):
        if self.cgfwNatRuleHandle is not None:
            WrtUtil.nftDeleteRule("fpemud-wrt", "natpost", self.cgfwNatRuleHandle)
            self.cgfwNatRuleHandle = None
